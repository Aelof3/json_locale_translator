...
parser.add_argument('-L', '--list', action='store_true', help='List all available languages (or any from google translate that are missing)')
...
# load source file
jsondata = json.load(open(dest_path + '/' + source_file))
...
def main():
    for i in range(len(language_list)):
        ...
        if language_code + '.json' in os.listdir(dest_path):
            jsonlangdata = json.load(open(dest_path + '/' + language_code + '.json'))
        ...
        newfile = open(dest_path + '/' + language_code + '.json', 'w')
